...
group_id    explicit group/recording identifier for test_real (optional, aliases: recording_id)
```
...
The group identifier for `test_real` is taken from manifest fields `group_id` or `recording_id` when present. If none of these are present, it is completely optional and the script will fallback to the first matching utterance-ID pattern:
...
sampling. If a dataset is already split, the user can provide an explicit `split` field in the manifest rows (with values `train` or `test`).
...
Realistic evaluation points built from held-out speaker/group material. Each
speaker/group is processed with 512-word windows and then aggregated back
to one score per speaker/group by a reference-word-weighted mean of the window
predictions.
...
same number of original utterances. For a multi-dataset run, the segmentation variants count is computed and run separately for each language/dataset. This ensures that every language generates the target number of segments (e.g. 8000), even if their source utterance counts differ drastically. The target corresponds to:
...
The implementation should estimate the number of variants for each dataset separately from a short pre-segmentation pass. For each dataset `i`:

```text
estimated_segments_per_variant_i =
    number of accepted segments produced by one random segmentation on dataset i

segment_variants_i =
    max(1, round(target_segments_per_language / estimated_segments_per_variant_i))
```
...
with neighbouring held-out utterances from the same speaker/group.
...
scatter plot shape
test_real group-level MAE
test_real group-level Pearson correlation
```
...
after aggregating 512-word window predictions to speaker/group-level points.
...
speaker/group group is converted into approximately 512-reference-word windows.
...
Each window is predicted independently. The final speaker/group-level estimate
...
The corresponding ground-truth group accuracy is computed by the same weighted
